...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (!id) {
      return NextResponse.json({ ok: false, message: "Hiányzó figyelmeztetés azonosító." }, { status: 400 });
    }

    const { error } = await auth.admin.from("vehicle_warnings").delete().eq("id", id);
