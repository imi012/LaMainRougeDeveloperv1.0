...
export async function POST(req: Request) {
  try {
    ...
    const del = await admin.from("leadando_submissions").delete().eq("id", id);
    if (del.error) {
      console.error("leadando delete error:", del.error);
      return NextResponse.json({ ok: false, message: "Nem sikerült törölni." }, { status: 500 });
    }

    await writeAdminAuditLog({
      actor_user_id: auth.userId,
      action: "leadando_delete",
    ...
    console.error("leadando delete fatal:", e);
